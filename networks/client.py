...
class Client:
    ...
    def receive(self):
        ...
        while True:
            recv_data = self._socket.recv(10000)
            data = pickle.loads(recv_data)
            # is_good, str_type, pos, size = data[0], data[1], data[2], data[3]
            # Si le 1er est None, c'est le signal que c'était le test des coordonnées
            '''
            if data[0] is None:
                print("réponse pour les coords :")
                if not data[1]:
                    print("-> not good")
                    self._interface._cancel_current_wall()
                else:
                    print("-> good")
                    # ne rien faire je crois ?
                    # self._interface._cancel_current_wall()
                    pass
            else:
            '''
